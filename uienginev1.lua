...
function UILibrary:CreateUI(player, modules, moduleFunctions)
    ...
    createInstance("UICorner", titleBar, "Corner").CornerRadius = UDim.new(0, 10)
    ...
    modeFrame.Visible = false  -- Disabled since we’re removing mode dropdown

    local settingsFrame = createInstance("ScrollingFrame", rightFrame, "SettingsFrame")
    settingsFrame.Size = UDim2.new(1, -16, 1, -50)  -- Adjusted height without mode frame
    settingsFrame.Position = UDim2.new(0, 8, 0, 45)
    ...
    function gui.showSettings(moduleData, index)
        ...
        modeFrame.Visible = false  -- Hide mode frame since we’re not using it
        settingsFrame.Position = UDim2.new(0, 8, 0, 45)  -- Adjust position without mode frame
        ...
        if moduleData.sliderValue and moduleData.sliderMin and moduleData.sliderMax then
            ...
            sliderLabel.Text = "Value: " .. (moduleData.sliderValue or 0) .. " (" .. (moduleData.sliderMin or 0) .. "-" .. (moduleData.sliderMax or 100) .. ")"
            ...
            local fillPercent = (moduleData.sliderValue - (moduleData.sliderMin or 0)) / ((moduleData.sliderMax or 100) - (moduleData.sliderMin or 0))
            sliderFill.Size = UDim2.new(math.clamp(fillPercent, 0, 1), 0, 1, 0)
            ...
            sliderButton.Position = UDim2.new(math.clamp(fillPercent, 0, 1), -12.5, 0, 0)
            ...
            UserInputService.InputChanged:Connect(function(input)
                if draggingSlider and input.UserInputType == Enum.UserInputType.MouseMovement then
                    ...
                    local newValue = math.floor((moduleData.sliderMax or 100) - (moduleData.sliderMin or 0)) * percent + (moduleData.sliderMin or 0)
                    moduleData.sliderValue = newValue
                    modules[index] = moduleData
                    sliderFill.Size = UDim2.new(percent, 0, 1, 0)
                    sliderButton.Position = UDim2.new(percent, -12.5, 0, 0)
                    sliderLabel.Text = "Value: " .. newValue .. " (" .. (moduleData.sliderMin or 0) .. "-" .. (moduleData.sliderMax or 100) .. ")"
                end
            end)
